  ...
  function updateEmploymentStatus(status: OnboardingAnswers["employmentStatus"]) {
    setAnswers((current) => ({
      ...
      incomeBand:
        status === "unemployed"
          ? "not_applicable"
          : current.incomeBand === "not_applicable"
            ? "30_50k"
            : current.incomeBand,
    }));
  }

  function updateRevolvingCredit(hasRevolvingCredit: boolean) {
    setAnswers((current) => ({
      ...current,
      hasRevolvingCredit,
      utilisationPct: hasRevolvingCredit ? current.utilisationPct : null,
    }));
  }
    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60">
      ...
      {step === 2 && (
        <Field title="What is your housing situation?" hint="This gives context to your profile, not a judgement.">
          <Select ariaLabel="Housing situation" value={answers.housingStatus} onChange={(value) => setAnswers({ ...answers, housingStatus: value as OnboardingAnswers["housingStatus"] })} options={["owner", "mortgage", "rent", "family", "other"]} />
        </Field>
      )}

      {step === 3 && (
        <Field title="Are you on the electoral roll at your current address?" hint="This can help with identity and address matching.">
          <YesNo value={answers.electoralRoll} onChange={(value) => setAnswers({ ...answers, electoralRoll: value })} />
        </Field>
      )}

      {step === 4 && (
        <Field title="Do you already have a credit card or other revolving credit?" hint="If yes, tell us the percentage of your total available credit that you are currently using.">
          <YesNo value={answers.hasRevolvingCredit} onChange={updateRevolvingCredit} />
          {answers.hasRevolvingCredit && (
            <label className="block text-sm font-bold text-slate-700">
              Credit utilisation (%)
              <input
                ...
                onChange={(e) => setAnswers({ ...answers, utilisationPct: e.target.value === "" ? null : Number(e.target.value) })}
              />
      ...
      {step === 5 && (
        <Field title="Any missed payments in the last 12 months?" hint="And if you use revolving credit, do you protect payments with a direct debit?">
          <input aria-label="Missed payments" className="field" type="number" min="0" value={answers.missedPaymentsLast12m} onChange={(e) => setAnswers({ ...answers, missedPaymentsLast12m: Number(e.target.value) })} />
          {answers.hasRevolvingCredit && <YesNo label="Direct debit set up" value={answers.hasDirectDebitForCredit} onChange={(value) => setAnswers({ ...answers, hasDirectDebitForCredit: value })} />}
        </Field>
      )}

      {step === 6 && (
        <Field title="How many hard credit applications in the last 6 months?" hint="An estimate is fine.">
          <input aria-label="Hard applications" className="field" type="number" min="0" value={answers.hardApplicationsLast6m} onChange={(e) => setAnswers({ ...answers, hardApplicationsLast6m: Number(e.target.value) })} />
        </Field>
      )}

      {step === 7 && (
        <Field title="Ready for your first mission?" hint="We’ll use these answers to choose one explainable next best move.">
      ...
      <div className="mt-8 flex gap-3">
        ...
        {step < 7 ? (
          <button data-testid="next" disabled={step === 0 && !answers.dateOfBirth} className="ml-auto rounded-2xl bg-violet-600 px-5 py-3 font-bold text-white disabled:opacity-40" onClick={() => setStep(step + 1)}>Next</button>
        ) : (
...
function Select({ ariaLabel, value, onChange, options }: { ariaLabel: string; value: string; onChange: (value: string) => void; options: string[] }) {
  return <select aria-label={ariaLabel} className="field w-full rounded-2xl border border-slate-200 bg-white px-4 py-3" value={value} onChange={(e) => onChange(e.target.value)}>{options.map((option) => <option key={option} value={option}>{option.replaceAll("_", " ")}</option>)}</select>;
}

function YesNo({ value, onChange, label }: { value: boolean; onChange: (value: boolean) => void; label?: string }) {
