...
import {
  ...
  type RecoveryContextConfirmation,
...
async function publishedReassessment(
...
async function publishedAlternativeRoutes(
  admin: SupabaseClient,
  partnerId: string,
  productCategory: RecoveryProductCategory,
  canonicalCode: string,
  ...
  const { data, error } = await admin
    ...
    .select("id,version,route_priority")
    .eq("partner_id", partnerId)
    .eq("product_category", productCategory)
    .eq("canonical_code", canonicalCode)
    .eq("lifecycle", "published")
...
export interface RecoveryPolicyActivationInput {
...
export async function buildRecoveryPolicySnapshotForActivation(
  admin: SupabaseClient,
  input: RecoveryPolicyActivationInput,
  ...
  for (const externalCode of reasonCodes) {
    ...
    const [template, reassessment, alternativeRoutePolicyIds] = await Promise.all([
      publishedTemplate(admin, canonical.code),
      publishedReassessment(admin, input.partnerId, input.productCategory, canonical.code),
    ...
    reasons.push({
      ...
      reassessmentRuleId: reassessment?.id ?? null,
      reassessmentRuleVersion: reassessment?.version ?? null,
      alternativeRoutePolicyIds,
    });
  }

  return buildRecoveryPolicySnapshot({
    partnerId: input.partnerId,
    productCategory: input.productCategory,
    capturedAt: now.toISOString(),
    contextConfirmation: input.contextConfirmation,
    customerCorrectionCode: input.customerCorrectionCode,
    partnerReasons: reasons,
  });
}

// Retained for compatibility with non-redemption callers that intentionally
// need build + persist as two server-side operations. Partner handoff redemption
// uses buildRecoveryPolicySnapshotForActivation and persists the exact result in
// the atomic consume + journey + snapshot RPC instead.
export async function resolveRecoveryPolicyForActivation(
  admin: SupabaseClient,
  input: RecoveryPolicyActivationInput & { recoveryJourneyId: string },
): Promise<RecoveryPolicySnapshot> {
  const snapshot = await buildRecoveryPolicySnapshotForActivation(admin, input);
