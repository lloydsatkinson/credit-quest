...
V2.3 extends the existing Credit Quest recovery capability with governed lender decline context while keeping the existing Credit Quest customer strategy engines authoritative.

- A governed **canonical decline taxonomy** maps approved lender reason codes to bounded risk families, recovery treatments and solveability classes. Unknown or unmapped lender reasons remain unknown; Credit Quest does not reverse-engineer an undisclosed score cut-off into a claimed root cause.
- Published mappings, recovery templates, reassessment rules and alternative-route policies are versioned and immutable. Published policy can move only through the audited `published → retired` lifecycle transition, and retired policy content remains immutable. An activated recovery journey receives an immutable **multi-reason policy snapshot**, and the deterministic multi-barrier resolver applies the approved blocker precedence across primary, secondary and parallel reasons.
- Policy may contain a **lender-configurable X-month wait or threshold** for the lender's own rule. Those lender parameters are **not disclosed to customers**, cannot change Safe Mode, Passport or mission ranking, and existing **Application Readiness remains authoritative**.
- The internal **Journey Builder** and production-equivalent **Simulator** are admin-authenticated controls. The Simulator reuses the same barrier resolver and three-valued condition evaluator as production instead of creating a second decision engine.
- `/lender` is a partner- and pilot-scoped **read-only lender console**. It reports aggregate canonical decline, cohort and funnel intelligence only; customer PII, Support Needs/vulnerability detail and raw financial values are outside that reporting boundary, and lender users cannot edit or publish recovery policy.
- Lender reporting uses **historical Ready-to-Check** semantics: once a governed journey first reached Ready-to-Check, a later change in current readiness cannot erase that historical event. Unavailable measurements remain unavailable rather than being converted to zero.
- An **alternative eligibility** route is optional and downstream of Credit Quest readiness. It requires an adult customer, normal safety mode, complete evidence, independent `ready_to_check`, explicit customer continuation, a routeable canonical reason and a valid server-owned contract. Negative disposable income, structural blockers, restricted or unknown reasons, and Safe Mode suppress the route.
- **fraud/AML** and other restricted security decisions remain a restricted non-recovery class. They do not enter the normal recovery mission chain, reapplication path or alternative-credit route.
- V2.3 keeps the existing five Recovery Experience states, Credit Passport, Mission engine, Application Readiness, Academy and seven-card Quest Feed. Lender policy is context downstream of those authorities, not a replacement for them.
- The deterministic 100-customer synthetic pilot covers representative single- and multi-decline scenarios using the real resolver and route gates. Synthetic records are explicitly synthetic and do not represent real lender or customer outcomes.
- V2.3 persistence is additive through `017_v2_3_decline_policy.sql`, `018_v2_3_multi_reason_snapshots.sql`, `019_v2_3_lender_pilot_reporting.sql`, `020_v2_3_policy_retirement.sql` and `021_v2_3_atomic_policy_snapshot_binding.sql`. Migration `020` adds the audited policy-retirement lifecycle; migration `021` makes handoff consumption, journey creation and the exact immutable policy snapshot one atomic activation transaction. V2.0d recovery privilege and credential-vault hardening remain in migrations `015` and `016`.
- All V2.3 dark defaults remain **OFF**:
...
Passing V2.3 CI does not merge or deploy the feature, apply production DDL, enrol a production lender pilot, enable partner intake or Return-to-Origin, send email, or activate a live regulated referral path. Those remain separate release-gated actions.

V2.3 design and implementation are documented in:

- `docs/superpowers/specs/2026-09-09-credit-quest-v2-3-lender-decline-recovery-design.md`
- `docs/superpowers/plans/2026-09-09-credit-quest-v2-3-lender-decline-recovery.md`